...
def process_xml_files_list(xml_file_paths, debug=False):
    ...
    for i, xml_path in enumerate(xml_file_paths, 1):
        try:
            ...
            processed_path = convert_raw_to_parsed_path(xml_path)
...
def convert_raw_to_parsed_path(raw_xml_path):
    """
    Convert a raw XML file path to the corresponding parsed JSON file path.
    
    Args:
        raw_xml_path: Path like ../data/raw/EPO/EPRTBJV.../folder/file.xml
        
    Returns:
        parsed path like ../data/parsed/EPO/EPRTBJV.../folder/file.json
    """
    ...
    # Replace 'raw' with 'parsed' in the path
    path_parts = list(raw_path.parts)
    
    # Find and replace 'raw' with 'parsed'
